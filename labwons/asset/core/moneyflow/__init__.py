...
from labwons.asset.core.moneyflow import data, traces
...
class moneyflow(object):
    ...
    def figure(self, **kwargs) -> Figure:
        ...
        fig.add_trace(row=3, col=1, trace=self.trac.mfi)
        ...
        fig.add_trace(row=4, col=1, trace=self.trac.cmf)
        ...
        fig.add_trace(row=5, col=1, trace=self.trac.obv)

        fig.update_layout(title=f"{self.meta['name']}({self.meta.name}): Money Flow", **kwargs)
        fig.update_yaxes(row=1, col=1, title=f"Price [{self.meta.currency}]")
        fig.update_yaxes(row=2, col=1, title=f"Vol.")
        fig.update_yaxes(row=3, col=1, title=f"MFI [%]")
        fig.update_yaxes(row=4, col=1, title=f"CMF [-]")
        fig.update_yaxes(row=5, col=1, title=f"OBV [-]")
        return fig
