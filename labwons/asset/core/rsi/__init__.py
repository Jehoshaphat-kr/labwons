...
from labwons.asset.core.rsi import data, traces
from labwons.common.charts import r5c1nsy
...
class rsi(object):
    ...
    def __init__(self, ohlcv:ohlcv, meta:Series):
        self.ohlcv = ohlcv
        self.data = data.gen(ohlcv.data)
        self.meta = meta
        self.trac = traces.traces(self.data)
        return
    ...
    def figure(self, **kwargs) -> Figure:
        ...
        fig.add_trace(row=3, col=1, trace=self.trac.rsi)
        ...
        fig.add_trace(row=4, col=1, trace=self.trac.stoch_osc)
        fig.add_trace(row=4, col=1, trace=self.trac.stoch_osc_sig)
        ...
        fig.add_trace(row=5, col=1, trace=self.trac.stoch_rsi)
        fig.add_trace(row=5, col=1, trace=self.trac.stoch_rsi_k)
        fig.add_trace(row=5, col=1, trace=self.trac.stoch_rsi_d)
        ...
        fig.update_layout(title=f"{self.meta['name']}({self.meta.name}): Relative Strength", **kwargs)
        fig.update_yaxes(row=1, col=1, title=f"Price [{self.meta.currency}]")
        fig.update_yaxes(row=2, col=1, title=f"Vol.")
        fig.update_yaxes(row=3, col=1, title=f"RSI[%]")
        fig.update_yaxes(row=4, col=1, title=f"OSC[%]")
        fig.update_yaxes(row=5, col=1, title=f"S-RSI[%]")
        return fig
