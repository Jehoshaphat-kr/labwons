...
class stock(object):
    ...
    class _two_frames_(DataFrame):
        __prop__ = (
            ("Y", "Q"),
            ("ThisYear", "NextYear")
        )
        def __init__(self, **kwargs):
            if not tuple(kwargs.keys()) in self.__prop__:
                raise KeyError("Wrong format for <class; multiFrames>")
    ...
    def abstract(self) -> DataFrame:
        ...
        if not hasattr(self, "_abstract"):
            yy = 11 if self.url.gb == 'D' else 14
            qq = 12 if self.url.gb == 'D' else 15
            self.__setattr__("_abstract", self._two_frames_(Y=_get_(yy), Q=_get_(qq)))
    ...
    def analogy(self) -> DataFrame:
        ...
        if not hasattr(self, '_analogy'):
            data = web.list(self.url.naver)[4]
    ...
    def benchmarkMultiples(self) -> DataFrame:
        ...
        """
        json = web.json(self.url.benchmarkMultiples)
    ...
    def cashFlow(self) -> DataFrame:
        ...
        if not hasattr(self, '_cashflow'):
            self.__setattr__('_cashflow', self._two_frames_(Y=_get_(4), Q=_get_(5)))
    ...
    def consensusPrice(self) -> DataFrame:
        ...
        data = web.data(self.url.consensusPrice, "CHART")
    ...
    def consensusProfit(self) -> DataFrame:
        ...
        def _get_(url:str) -> DataFrame:
            data = web.data(url, "CHART").replace('-', numpy.nan)[cols.keys()]
            data = data.rename(columns=cols)
            return data.set_index(keys="기말").astype(float)
        return self._two_frames_(Y=_get_(self.url.consensusAnnualProfit), Q=_get_(self.url.consensusQuarterProfit))


    @property
    def consensusTendency(self) -> DataFrame:
        """
        :return:
        ...
        def _get_(url:str) -> DataFrame:
            ...
            for col in data:
                data[col] = data[col].apply(str2num)
            return data
        return self._two_frames_(
            ThisYear=_get_(self.url.consensusForward1Y), NextYear=_get_(self.url.consensusForward2Y)
        )
    ...
    def expenses(self) -> DataFrame:
        ...
        :return:
            ...
            2022/12     10.69       14.06
        """
        json = web.json(self.url.expenses)
        def exp(period: str) -> Series:
            ...
            return concat
        return self._two_frames_(Y=exp('Y'), Q=exp('Q'))
    ...
    def financialStatement(self) -> DataFrame:
        ...
        return self._two_frames_(Y=_get_("Y"), Q=_get_("Q"))


    @property
    def financialStatementSeparate(self) -> DataFrame:
        """
        :return:
            [별도]
        ...
        cutter = ['계산에 참여한 계정 펼치기', '(', ')', '*', '&nbsp;', ' ', " "]
        def _get_(period: str) -> DataFrame:
            data = web.list(self.url.separateFinance)[{"Y": 2, "Q": 3}[period]]
            data = data.set_index(keys=[data.columns[0]])
            data = data.drop(columns=[col for col in data if not col.startswith('20')])
            data.index.name = None
            data.columns = data.columns.tolist()[:-1] + [f"{data.columns[-1][:4]}/{int(data.columns[-1][-2:]) // 3}Q"]
    ...
    @property
    def foreignRate(self) -> DataFrame:
        """
        :return:
                                    3M              1Y              3Y
                           종가   비중     종가   비중     종가   비중
        ...
        objs = {}
        for u in [self.url.foreignRate3M, self.url.foreignRate1Y, self.url.foreignRate3Y]:
    ...
    def marketCap(self) -> Series:
        ...
        if not hasattr(self, '_marketcap'):
            ...
            self.__setattr__('_marketcap', Series(index=cap.index, data=cap['시가총액'], dtype=int)  / 100000000)
    ...
    def multipleBand(self) -> DataFrame:
        ...
        """
        json = web.json(self.url.multipleBands)
        def _get_(key: str) -> DataFrame:
    ...
    def multiples(self) -> DataFrame:
        ...
        data = web.list(self.url.invest)[3]
    ...
    def multiplesTrailing(self) -> Series:
        ...
        :return:
            ...
            dtype: float64
        """
        data = web.list(self.url.naver)[8]
    ...
    def products(self) -> DataFrame:
        ...
        """
        json = web.json(self.url.products)
    ...
    def shareHolders(self) -> Series:
        ...
        :return:
            ...
            dtype: float64
        """
        data = web.data(self.url.shares).replace("", numpy.nan)
    ...
    def shortBalance(self) -> DataFrame:
        ...
        data = web.data(self.url.shortBalance, "CHART").rename(columns=cols)[cols.values()].set_index(keys='날짜')
        data.index = pandas.to_datetime(data.index)
    ...
    def shortSell(self) -> DataFrame:
        ...
        data = web.data(self.url.shortSell, "CHART").rename(columns=cols).set_index(keys='날짜')
        data.index = pandas.to_datetime(data.index)
        return data.replace("", numpy.nan).astype(float)
...
class etf(price):
    def __init__(self, ticker:str):
        super().__init__(ticker=ticker, country="KOR")
        self.url = urls(ticker)
        return
    ...
    def sectorWeights(self) -> DataFrame:
        ...
        """
        n, base = 100, ""
        src = str(web.html(self.url.etf)).split('\r\n')
        while n < len(src):
