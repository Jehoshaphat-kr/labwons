...
from plotly.graph_objects import Scatter, Bar, Candlestick
...
def lineTY(data: pandas.Series, drop: bool = True, **kwargs) -> Scatter:
    if drop:
        data = data.dropna()
    ...
    return _overwrite(Scatter, trace, **kwargs)

def lineXY(data: pandas.Series, drop: bool = True, **kwargs) -> Scatter:
    if drop:
        data = data.dropna()
    name = kwargs['name'] if 'name' in kwargs else '/'.join(data.name) if isinstance(data.name, tuple) else data.name
    unit = kwargs['unit'] if 'unit' in kwargs else ""
...
def barTY(data: pandas.Series, drop: bool = True, **kwargs) -> Bar:
    if drop:
        data = data.dropna()
    name = kwargs['name'] if 'name' in kwargs else '/'.join(data.name) if isinstance(data.name, tuple) else data.name
    unit = kwargs['unit'] if 'unit' in kwargs else ""
...
def barXY(data: pandas.Series, drop: bool = True, **kwargs) -> Bar:
    if drop:
        data = data.dropna()
    name = kwargs['name'] if 'name' in kwargs else '/'.join(data.name) if isinstance(data.name, tuple) else data.name
    unit = kwargs['unit'] if 'unit' in kwargs else ""
...
def scatterTY(data: pandas.Series,drop: bool = True, **kwargs) -> Scatter:
    if drop:
        data = data.dropna()
    name = kwargs['name'] if 'name' in kwargs else '/'.join(data.name) if isinstance(data.name, tuple) else data.name
    unit = kwargs['unit'] if 'unit' in kwargs else ""
...
def candleStick(data: pandas.DataFrame, name:str, drop: bool = True, **kwargs) -> Candlestick:
    ...
    if drop:
        data = data.dropna()
    yformat = ".2f" if isinstance(data[data.columns[0]], float) else ",d"
    trace = Candlestick(
        name=name,
        x=data.index,
        open=data['open'],
    ...
    return _overwrite(Candlestick, trace, **kwargs)
